...
class Classifier(nn.Module):
    def __init__(self, nfeatr):
        ...
        self.dense1 = nn.Linear(nfeatr, 2)
...
class MISICR(nn.Module):
    def __init__(self, device):
        super(MISICR, self).__init__()
        self.device = device
        self.E_1 = LocalEncoder_EEGNet(fs=250, num_ch=62, num_time=425)
        self.model_V = Decomposer(nfeat=16)
        self.E_g = GlobalEncoder_EEGNet(num_ch=62, num_time=425, nfeatl=16)
        self.model_C = Classifier(nfeatr=208)
        self.mine = MINE(nfeatr=1680, nfeati=1680)
        self.mine_glo = Global_disc_EEGNet(nfeatl=16, nfeatg=208, num_ch=62)
        self.mine_loc = Local_disc_EEGNet(nfeatl=16, nfeatg=208, nfeatl2=105, num_ch=62)
    def forward(self, x):
        f_1 = self.E_1(x)
        f_re, f_ir = self.model_V(f_1)
        f_g = self.E_g(f_re)
        y = self.model_C(f_g)
        device = torch.device(self.device)
        f_re_ = torch.reshape(f_re, (f_re.shape[0], -1))
        f_ir_ = torch.reshape(f_re, (f_ir.shape[0], -1))

        ishuffle = torch.index_select(f_re_, 0, torch.randperm(f_re_.shape[0]).to(device))
        djoint = self.mine(f_re_, f_ir_)
        dmarginal = self.mine(f_re_, ishuffle)

        gshuffle = torch.index_select(f_g, 0, torch.randperm(f_g.shape[0]).to(device))
        gjoint = self.mine_glo(f_re, f_g)
        gmarginal = self.mine_glo(f_re, gshuffle)

        ijoint = self.mine_loc(f_re, f_g)
        imarginal = self.mine_loc(f_re, gshuffle)
        return y, djoint, dmarginal, gjoint, gmarginal, ijoint, imarginal
