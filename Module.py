...
class Group_cls(nn.Module):
    def __init__(self, drop_out=0.5):
        super(Group_cls, self).__init__()
        ...
        self.block_2 = nn.Linear(2236, 27)
...
class model_4(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.out_put == 'all':
            return pred_task, emo_enc, task_enc, joint, marginal, group_id
        if self.out_put == 'pred':
            return pred_task
        if self.out_put == 'emo':
            return  emo_enc
